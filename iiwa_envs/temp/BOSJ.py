...
def obj(params, table, bagdata) -> np.ndarray:
    ...
    for i in range(params.shape[0]):
        X = [
             params.iloc[i].get('l_res'),
             params.iloc[i].get('l_f'),
             params.iloc[i].get('s_res'),
             params.iloc[i].get('s_f')
...
#  [t_lateral_f, left_right_rim_res, left_rim_f, four_side_rim_res, four_side_rim_latf, angvel]  [5,]
    param_list = [
        {'name': 'l_res', 'type': 'num', 'lb': .5, 'ub': .95},
        {'name': 'l_f', 'type': 'num', 'lb': 0., 'ub': .8},
        {'name': 's_res', 'type': 'num', 'lb': .5, 'ub': .95},
        {'name': 's_f', 'type': 'num', 'lb': 0., 'ub': .8}
  ...
  #{'name': 'latf', 'type': 'num', 'lb': .01, 'ub': .09}
    ...
    rec = pd.DataFrame({
                        ...
                        'l_res': [.9, .7, .95, .59, .67, .75, .62, .8],
                        'l_f': [.1, .3, .45, .6, .01, .4, .6, .7],
                        's_res': [.61, .73, .645, .86, .81, .54, .86, .937],
                        's_f': [.1, .3, .45, .2, .01, .4, .16, .07]
    ...
    params = []
    n_sugg = 15
    fileHeader = ["iters", "l_res", "l_f", "s_res", "s_f", "objs" ]
    with open("/home/hszlyw/PycharmProjects/ahky/iiwa_envs/results_v1/zum Ausarbeitung2/all_dis/" + "all_dis_s90", 'w') as csvfile:
        ...
        for iter in range(70):
            ...
            for j in range(n_sugg):
                ...
                cw.writerow([
                    str(iter),
                    str(rec.l_res.values[j]),
                    str(rec.l_f.values[j]),
                    str(rec.s_res.values[j]),
                    str(rec.s_f.values[j]),
            ...
            print("Current: ", "objective: {} ".format(mean_obj[0, 0]),
                  ...
                  "s_res: {}, "
                  "s_f: {}, ".format(
                                                 opt.X.iloc[opt.y.argmin()]['l_res'],
                                                 opt.X.iloc[opt.y.argmin()]['l_f'],
                                                 opt.X.iloc[opt.y.argmin()]['s_res'],
                                                 opt.X.iloc[opt.y.argmin()]['s_f']
